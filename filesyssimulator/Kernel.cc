...
FileDescriptor ** Kernel::deletedFiles;
int Kernel::MAX_OPEN_FILES;
int Kernel::MAX_DELETED_FILES;
map<int, int> blockInodeMap;
...
int Kernel::close(int fd)
{
	...
	for( int i = 0 ; i < MAX_OPEN_FILES ; i ++ )
	{
        ...
        for(int j=0;j<MAX_DELETED_FILES;j++)
        {
            if(deletedFiles[j] != NULL && deletedFiles[j] == process.openFiles[fd])
            {
                IndexNode * indexNode = deletedFiles[j]->getIndexNode();
                freeIndexNode(indexNode,deletedFiles[j]->getIndexNodeNumber());
                deletedFiles[j] = NULL;
                break;
            }
        }
...
//			cout << "Next : " << nextDirectoryEntry.toString() << endl;
	...
	else
	{
		...
		int blocks = (currIndexNode.getSize() + blockSize-1) / blockSize;
                ...
                freeIndirectBlocks(&currIndexNode);
...
//
        ...
        for(int i=0;i<MAX_OPEN_FILES;i++)
	{
        ...
        //create deleted files array
        deletedFiles = new FileDescriptor*[MAX_DELETED_FILES];
        for(int i=0;i<MAX_DELETED_FILES;i++)
	{
		deletedFiles[i] = NULL;
	}
...
int Kernel::link(char *oldFilePath, char *newFilePath)
{
    IndexNode indexNode;
    short inum = findIndexNode(oldFilePath, indexNode);

    // get the full path
    char * fullPath = getFullPath(newFilePath);
    ...
    while(1)
    {
        if(token != NULL)
        {
            ...
            if((currIndexNode.getMode()&S_IFMT) != S_IFDIR)
            {
                // return (ENOTDIR) if a needed dir is not a directory
                process.errno = ENOTDIR ;
                return EXIT_FAILURE;
            }
    ...
    int status = 0;
    DirectoryEntry newDirectoryEntry(inum , name);
    DirectoryEntry currentDirectoryEntry;
...
int Kernel::freeIndexNode(IndexNode* indexNode, short inum)
{
    FileSystem * fileSystem = openFileSystems;
    int blockSize = fileSystem->getBlockSize();
    int blocks = (indexNode->getSize() + blockSize-1) / blockSize;

    freeIndirectBlocks(indexNode);
...
int Kernel::unlink(char *filePath)
{
    ...
    IndexNode indexNode;
    short inum = findIndexNode(filePath, indexNode);
    ...
    if (indexNode.getNlink() == 0) {
        for(int i=0;i<MAX_OPEN_FILES;i++)
        {
            if(openFiles[i] != NULL)
            {
                FileDescriptor* file = openFiles[i];
                if(file->getIndexNodeNumber() == inum)
                {
                    //File is open. Add file to the deleted list - The file will be deleted when it is closed
                    for(int j=0;j<MAX_DELETED_FILES;j++)
                    {
                        if(deletedFiles[j]==NULL)
                        {
                            deletedFiles[j] = file;
                            isOpen = true;
                            break;
    ...
    {
        perror(PROGRAM_NAME);
        cout << PROGRAM_NAME << ": unable to open directory for writing in unlink" << endl;
        return EXIT_FAILURE;
    }
    ...
    while(true)
    {
        ...
        {
            // if current item == new item, break.
    ...
    while(status>0)
    {
        ...
        if(status == 0) {
            ...
            if( seek_status < 0 )
            {
                cout << PROGRAM_NAME << ": error during seek in unlink" ;
                return EXIT_FAILURE;
            }
...
int Kernel::freeIndirectBlocks(IndexNode* indexNode)
{
    FileSystem * fileSystem = openFileSystems;
    int blockSize = fileSystem->getBlockSize();
    int nblocks = blockSize / 3;
    int indirectBlockOffset = indexNode->getIndirectBlock();
    if(indirectBlockOffset == FileSystem::NOT_A_BLOCK)
    {
        return 0;
    }
    ...
    fileSystem->read(temp_bytes, fileSystem->getDataBlockOffset() + indirectBlockOffset);

    for(int i = 0; i < nblocks; i++ )
    {
        int b2 = temp_bytes[3*i] & 0xff;
        int b1 = temp_bytes[3*i+1] & 0xff;
        int b0 = temp_bytes[3*i+2] & 0xff;
        int indirectBlockNumber = b2 << 16 | b1 << 8 | b0 ;

        if(indirectBlockNumber != FileSystem::NOT_A_BLOCK)
        {
            fileSystem->freeBlock(indirectBlockNumber);
            temp_bytes[3*i]   = (unsigned char)(FileSystem::NOT_A_BLOCK >> 16);
            temp_bytes[3*i+1] = (unsigned char)(FileSystem::NOT_A_BLOCK >> 8);
            temp_bytes[3*i+2] = (unsigned char)(FileSystem::NOT_A_BLOCK);
        }
    }
    fileSystem->write(temp_bytes, fileSystem->getDataBlockOffset() + indirectBlockOffset);
    return 0;
}

int Kernel::defrag()
{
    int status = defragPopulateMap("/", blockInodeMap);

    if(status == EXIT_FAILURE)
    {
        cout << "Error in populating defrag map." << endl;
        return EXIT_FAILURE;
    }

#if 0
    //Code to print blockInodeMap
    for (map<int,int>::iterator it=blockInodeMap.begin(); it!=blockInodeMap.end(); ++it)
    {
        cout << "block:" << it->first << "allocated to inode:" << it->second << endl;
    }
    ...
    fileSystem->defragMover(blockInodeMap);
...
int Kernel::defragPopulateMap(char *dirname, map<int,int> &inodeMap) {
    ...
    if( dir < 0 )
    {
        cout << "defrag: unable to open directory" << endl;
        return EXIT_FAILURE;
    }
    ...
    while(true) {
        ...
        if(status == 0)
        {
            ...
            int inum = currentDirectoryEntry.getIno();
            IndexNode indexNode;
            ...
            for( int i = 0 ; i < blocks ; i ++ )
            {
                ...
                if(i >= IndexNode::MAX_DIRECT_BLOCKS)
                {
                    inodeMap[address] = inum;

                    char * temp_bytes = new char[blockSize];
                    memset(temp_bytes, '\0', blockSize);
                    fileSystem->read(temp_bytes, fileSystem->getDataBlockOffset() + address);

                    int indirectAddress = (i - IndexNode::MAX_DIRECT_BLOCKS) * 3;

                    int b2 = temp_bytes[indirectAddress] & 0xff;
                    int b1 = temp_bytes[indirectAddress+1] & 0xff;
                    int b0 = temp_bytes[indirectAddress+2] & 0xff;
                    int indirectBlockNumber = b2 << 16 | b1 << 8 | b0 ;

                    address = indirectBlockNumber;
                }

                inodeMap[address] = inum;
            }

            //if the directory entry is a directory, populate blockInodeMap for this directory
            if((indexNode.getMode() & Kernel::S_IFMT ) == Kernel::S_IFDIR)
            {
                ...
                int s = defragPopulateMap(tempDirName, inodeMap);
                if (s == EXIT_FAILURE) {
                    cout << "error: some issue in defrag populate method" << endl;
                    return EXIT_FAILURE;
                } else {
                    continue;
                }
