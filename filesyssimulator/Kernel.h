...
//		static char * sys_errlist[32];
	...
	private:
		...
		static int MAX_DELETED_FILES;
		static FileDescriptor ** openFiles;
		static FileDescriptor ** deletedFiles;
		...
		static int freeIndirectBlocks(IndexNode* indexNode);
		static int freeIndexNode(IndexNode* indexNode, short inum);
                static int defragPopulateMap(char *dirname, map<int,int> &inodeMap);
};
