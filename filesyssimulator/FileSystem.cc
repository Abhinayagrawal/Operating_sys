...
void FileSystem::blockAllocationCheck(int inum, vector<int> *allocatedBlockList)
{
        IndexNode indexNode;
        readIndexNode(&indexNode, inum);
        ...
        for( int i = 0 ; i < blocks ; i ++ )
        {
            ...
            if(i >= IndexNode::MAX_DIRECT_BLOCKS)
            {
                ...
                read(temp_bytes, dataBlockOffset + address);
                ...
                allocatedBlockList->push_back(address);
                address = indirectBlockNumber;
            }

            loadFreeListBlock(address);
            bool allocated = freeListBitBlock->isBitSet(address % (blockSize * 8));
            //cout << "i" << i << "address:" << address << "bit:" << (address % (blockSize * 8)) << endl;

            if(!allocated)
            {
                cout << "Issue: Block " << i << " with address " << address <<
                    " is not allocated for inode number " << inum << endl;
            }
...
void FileSystem::blockFreeCheck(vector<int> allocatedBlockList)
{
    int dataBlockCount = blockCount - dataBlockOffset;
    for (int i = 0; i <= dataBlockCount;  ++i)
    {
        if(find(allocatedBlockList.begin(), allocatedBlockList.end(), i) == allocatedBlockList.end())
        {
            loadFreeListBlock(i);
            bool allocated = freeListBitBlock->isBitSet(i % (blockSize * 8));
            if(allocated)
            {
                cout << "Issue: Block " << i  << " is allocated but is not linked with any inode." << endl;
            }
        }
    }
}

void FileSystem::defragMover(map<int,int> &blockInodeMap)
{
	int dataBlockCount = blockCount - dataBlockOffset;
	int i=0, j=dataBlockCount-1;
	while(i<j)
	{
		loadFreeListBlock(i);
		bool frontAllocated = freeListBitBlock->isBitSet(i % (blockSize * 8));
		loadFreeListBlock(j);
		bool rearAllocated = freeListBitBlock->isBitSet(j % (blockSize * 8));

		if(frontAllocated || !rearAllocated)
		{
			if(frontAllocated)
				i++;
		...
		//Front is free and rear is full. Copy contents of rear to front
		char * bytes = new char[blockSize];
		memset(bytes, '\0', blockSize);

		read(bytes, dataBlockOffset + j);
		write(bytes, dataBlockOffset + i);
                
		loadFreeListBlock(i);
                freeListBitBlock->setBit(i % (blockSize * 8));
		freeBlock(j);

		int status = updateInode(blockInodeMap[j],j,i);
		blockInodeMap[i] = blockInodeMap[j];
		blockInodeMap.erase(j);

		if(status < 0)
		{
                    cout << "defragMover : Error in updating inode number " << blockInodeMap[j] << endl;
		}
                i++;
                j--;
	}
...
int FileSystem::updateInode(short inum, int oldBlockNum, int newBlockNum)
{
		IndexNode indexNode;
		readIndexNode(&indexNode,inum);
		int* tempDirectBlocks = indexNode.getDirectBlocks();
		for( int i = 0 ; i < IndexNode::MAX_DIRECT_BLOCKS ; i ++ )
		{
			if(tempDirectBlocks[i] == oldBlockNum)
			{
				indexNode.setBlockAddress(i , newBlockNum);
				writeIndexNode(&indexNode, inum);
				return 0;
			}
		}
		...
		read(temp_bytes, dataBlockOffset + indirectBlockOffset);
		...
		for(int i = 0; i < nblocks; i++ )
		{
				...
				if(indirectBlockNumber == oldBlockNum)
				{
						temp_bytes[3*i]   = (unsigned char)(newBlockNum >> 16);
						temp_bytes[3*i+1] = (unsigned char)(newBlockNum >> 8);
						temp_bytes[3*i+2] = (unsigned char)(newBlockNum);

						break;
				}
		}

		write(temp_bytes, dataBlockOffset + indirectBlockOffset);
		return 0;
}
