...
/**
 * Gets the indirect block for this IndexNode.
 * This is the block number of the indirect block.
 */
...
/**
 * Sets the indirect block for this IndexNode.
 * This is the block number of the indirect block.
 */
void IndexNode::setIndirectBlock(int newIndirectBlock)
{
	indirectBlock = newIndirectBlock;
}
...
void IndexNode::write(char * buffer, int offset)
{
	...
	buffer[offset+12+3*MAX_DIRECT_BLOCKS]     = (unsigned char)(indirectBlock >> 16);
	buffer[offset+12+3*MAX_DIRECT_BLOCKS+1]   = (unsigned char)(indirectBlock >> 8);
	buffer[offset+12+3*MAX_DIRECT_BLOCKS+2]   = (unsigned char)(indirectBlock);
...
void IndexNode::read(char * buffer , int offset)
{
	...
	// leave room for indirectBlock, doubleIndirectBlock, tripleIndirectBlock
	b2 = buffer[offset+12+MAX_DIRECT_BLOCKS*3] & 0xff ;
	b1 = buffer[offset+12+MAX_DIRECT_BLOCKS*3+1] & 0xff ;
	b0 = buffer[offset+12+MAX_DIRECT_BLOCKS*3+2] & 0xff ;
...
#if 0
int IndexNode::getMaxFileBlocks()
{
        return maxFileBlocks ;
}

void IndexNode::setMaxFileBlocks(int maxFileBlockCount)
{
        maxFileBlocks = maxFileBlockCount ;
}
#endif
